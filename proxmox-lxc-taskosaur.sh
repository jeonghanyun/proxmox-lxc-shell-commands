...
install_dependencies() {
    ...
    info "Installing Node.js 22, PostgreSQL, Redis, and build tools..."
...
setup_database() {
    ...
    # Start PostgreSQL
    pct exec "$CT_ID" -- bash -c "systemctl start postgresql"
    pct exec "$CT_ID" -- bash -c "systemctl enable postgresql"

    # Create database and user
    pct exec "$CT_ID" -- bash -c "sudo -u postgres psql -c \"CREATE USER taskosaur WITH PASSWORD '${TASKOSAUR_DB_PASSWORD}';\""
    pct exec "$CT_ID" -- bash -c "sudo -u postgres psql -c \"CREATE DATABASE taskosaur OWNER taskosaur;\""
    pct exec "$CT_ID" -- bash -c "sudo -u postgres psql -c \"GRANT ALL PRIVILEGES ON DATABASE taskosaur TO taskosaur;\""
...
setup_redis() {
    ...
    # Start Redis
    pct exec "$CT_ID" -- bash -c "systemctl start redis-server"
    pct exec "$CT_ID" -- bash -c "systemctl enable redis-server"
...
install_taskosaur() {
    ...
    # Clone repository
    info "Cloning Taskosaur repository..."
    pct exec "$CT_ID" -- bash -c "cd /opt && git clone https://github.com/Taskosaur/Taskosaur.git taskosaur"
...
EOF"
    ...
    info "Installing npm dependencies (this will take 5-10 minutes)..."
    pct exec "$CT_ID" -- bash -c "cd /opt/taskosaur && npm install --legacy-peer-deps 2>&1 | tail -20"
    ...
    # Build application
    info "Building Taskosaur (this will take 5-10 minutes)..."
    pct exec "$CT_ID" -- bash -c "cd /opt/taskosaur && npm run build 2>&1 | tail -20"
...
EOF"
    ...
    pct exec "$CT_ID" -- bash -c "systemctl daemon-reload"
    pct exec "$CT_ID" -- bash -c "systemctl enable taskosaur-backend taskosaur-frontend"
...
🔑 INITIAL CREDENTIALS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Check seeded admin credentials in the database or logs
Run: pct exec ${CT_ID} -- cat /opt/taskosaur/README.md
...
main() {
    info "Starting Taskosaur LXC container creation..."
    echo ""
