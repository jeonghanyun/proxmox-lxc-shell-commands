...
# Proxmox LXC Coolify Installation Script
# Description: Automated creation and configuration of Coolify self-hosted PaaS in LXC container
# OS: Debian 12 (Bookworm) - Auto-detected latest version
# Ports: Web UI 8000, SSH 22 (internal), Application ports as needed
...
CT_HOSTNAME=${CT_HOSTNAME:-"coolify"}                  # Container hostname
CT_CORES=${CT_CORES:-2}                               # CPU cores (minimum 2 recommended)
CT_MEMORY=${CT_MEMORY:-4096}                          # RAM in MB (minimum 4GB recommended)
CT_SWAP=${CT_SWAP:-2048}                              # Swap in MB
CT_DISK_SIZE=${CT_DISK_SIZE:-30}                      # Root disk size in GB (minimum 30GB recommended)
...
COOLIFY_WEB_PORT=${COOLIFY_WEB_PORT:-8000}            # Web UI port
...
check_resources() {
    ...
    if [[ $CT_MEMORY -lt 4096 ]]; then
        warn "Memory is set to ${CT_MEMORY}MB. Coolify recommends at least 4096MB (4GB)"
        warn "Proceeding anyway, but performance may be affected"
    fi

    if [[ $CT_CORES -lt 2 ]]; then
        warn "CPU cores set to ${CT_CORES}. Coolify recommends at least 2 cores"
        warn "Proceeding anyway, but performance may be affected"
    fi

    if [[ $CT_DISK_SIZE -lt 30 ]]; then
        warn "Disk size is ${CT_DISK_SIZE}GB. Coolify recommends at least 30GB"
...
#################################################################
# Coolify Installation Functions
#################################################################

install_coolify() {
    info "Installing Coolify in container $CT_ID..."
    info "This may take several minutes..."

    # Install additional dependencies
    info "Installing additional dependencies..."
    pct exec "$CT_ID" -- bash -c "DEBIAN_FRONTEND=noninteractive apt-get install -y -qq openssh-server"

    # Start SSH (required by Coolify)
    pct exec "$CT_ID" -- bash -c "systemctl enable ssh"
    pct exec "$CT_ID" -- bash -c "systemctl start ssh"

    # Run Coolify installer
    info "Running Coolify installer (this will take a while)..."
    pct exec "$CT_ID" -- bash -c "curl -fsSL https://cdn.coollabs.io/coolify/install.sh | bash" || {
    ...
    if pct exec "$CT_ID" -- bash -c "docker ps | grep -q coolify"; then
        success "Coolify installed and running"
    else
        warn "Coolify containers may still be starting..."
...
add_container_notes() {
    ...
    local notes="COOLIFY SELF-HOSTED PAAS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
🌐 COOLIFY ACCESS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Web UI:          http://${container_ip}:${COOLIFY_WEB_PORT}
...
🔧 COOLIFY MANAGEMENT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Status:          pct exec ${CT_ID} -- docker ps | grep coolify
Restart:         pct exec ${CT_ID} -- docker restart \$(docker ps -q)
Logs:            pct exec ${CT_ID} -- docker logs coolify -f
...
• First access: Create your admin account
• Recommended: Configure a domain for production use
...
display_info() {
    ...
    echo ""
    echo "================================================================="
    success "Coolify LXC Container Setup Complete!"
    echo "================================================================="
    echo ""
    ...
    echo ""
    echo "Coolify Access:"
    echo "  • Web UI:            http://${container_ip}:${COOLIFY_WEB_PORT}"
    echo "  • First Access:      Create your admin account"
    echo ""
    echo "Docker Management:"
    echo "  • List containers:   pct exec $CT_ID -- docker ps"
    echo "  • List images:       pct exec $CT_ID -- docker images"
    echo "  • View logs:         pct exec $CT_ID -- docker logs <container>"
    echo ""
    ...
    echo ""
    warn "⚠️  First Access: Go to http://${container_ip}:${COOLIFY_WEB_PORT}"
    warn "    and create your admin account to complete setup"
    echo ""
    echo "================================================================="
}
...
main() {
    info "Starting Coolify LXC container creation..."
    warn "Coolify requires a PRIVILEGED container with Docker"
    echo ""
